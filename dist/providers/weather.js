...
export function getLocaleUnitsParam(locale) {
    const loc = locale ?? Intl.DateTimeFormat().resolvedOptions().locale;
    return loc.includes('US') ? 'u' : 'm';
}
export function parseWeatherOutput(output) {
    const trimmed = output.trim();
    if (!trimmed)
        return null;
    if (trimmed.startsWith('Unknown') || trimmed.startsWith('ERROR') || trimmed.length > 30)
        return null;
...
export function fetchWeatherSync() {
    try {
