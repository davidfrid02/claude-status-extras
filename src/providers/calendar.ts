...
export function parseCalendarOutput(output: string): CalendarData | null {
  const trimmed = output.trim();
  if (!trimmed || trimmed === 'no_events') return null;
  const lines = trimmed.split('\n');
  if (lines.length < 2) return null;
  const title = lines[0];
  const minutesUntil = parseInt(lines[1], 10);
  if (Number.isNaN(minutesUntil)) return null;
...
export function formatNoMeetings(): string {
